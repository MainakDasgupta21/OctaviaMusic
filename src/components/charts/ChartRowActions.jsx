  ...
  >
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="bg-surface-2 border-white/10 text-ink min-w-[13rem]">
        <DropdownMenuItem onClick={() => onAddToPlaylist?.(entry)}>Add to playlist</DropdownMenuItem>
        <DropdownMenuItem onClick={() => onAddFavorite?.(entry)}>Add to favorites</DropdownMenuItem>
        <DropdownMenuItem onClick={() => onShare?.(entry)}>Share chart position</DropdownMenuItem>
        <DropdownMenuSeparator className="bg-white/10" />
        <DropdownMenuItem onClick={() => onGoAlbum?.(entry)}>Go to album</DropdownMenuItem>
        <DropdownMenuItem onClick={() => onGoArtist?.(entry)}>Go to artist</DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  </div>
);
